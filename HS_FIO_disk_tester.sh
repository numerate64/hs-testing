...
#
# Rev 1.5 10/15/24 - 
#   Replace references to m0 with data-cluster
#   Changed dsx nfs test to use unique directories for each DSX
#   Changed DSX remote volume mount to use v3
#
# Yet to do -
...
select fav in "${choices[@]}"; do
    case $fav in
        ...
        l_and_d "------------------------------------------------"
        salt -G 'pd-node-type:DSX' cmd.run "scp serviceadmin@data-cluster:$fio_file $fio_file"
        echo
        ...
        l_and_d "-------------------------------------------"
        salt -G 'pd-node-type:DSX' cmd.run "mount -v -t nfs -o v3,nolock $nfs_export /mnt/test"
        echo
        ...
        salt -G 'pd-node-type:DSX' cmd.run "mkdir -v /mnt/test/fio/\$HOSTNAME"
        echo
        ...
        salt -G 'pd-node-type:DSX' cmd.run "fio --output-format=json --directory=/mnt/test/fio/\$HOSTNAME --section=seq-read --output=/home/serviceadmin/$hssevar-$companyvar/fio-dsxnfs-seqread-$HOSTNAME-$DATESTAMP.json $fio_file"
        l_and_d ""
        l_and_d "Running sequential write test."
        salt -G 'pd-node-type:DSX' cmd.run "fio --output-format=json --directory=/mnt/test/fio/\$HOSTNAME --section=seq-write --output=/home/serviceadmin/$hssevar-$companyvar/fio-dsxnfs-seqwrite-$HOSTNAME-$DATESTAMP.json $fio_file"
        l_and_d ""
        l_and_d "Running sequential read-write test."
        salt -G 'pd-node-type:DSX' cmd.run "fio --output-format=json --directory=/mnt/test/fio/\$HOSTNAME --section=seq-rw --output=/home/serviceadmin/$hssevar-$companyvar/fio-dsxnfs-seqrw-$HOSTNAME-$DATESTAMP.json $fio_file"
        l_and_d ""
        l_and_d "Running random read test."
        salt -G 'pd-node-type:DSX' cmd.run "fio --output-format=json --directory=/mnt/test/fio/\$HOSTNAME --section=rand-read --output=/home/serviceadmin/$hssevar-$companyvar/fio-dsxnfs-randread-$HOSTNAME-$DATESTAMP.json $fio_file"
        l_and_d ""
        l_and_d "Running random write test."
        salt -G 'pd-node-type:DSX' cmd.run "fio --output-format=json --directory=/mnt/test/fio/\$HOSTNAME --section=rand-write --output=/home/serviceadmin/$hssevar-$companyvar/fio-dsxnfs-randwrite-$HOSTNAME-$DATESTAMP.json $fio_file"
        l_and_d ""
        l_and_d "Running random read-write test."
        salt -G 'pd-node-type:DSX' cmd.run "fio --output-format=json --directory=/mnt/test/fio/\$HOSTNAME --section=rand-rw --output=/home/serviceadmin/$hssevar-$companyvar/fio-dsxnfs-randrw-$HOSTNAME-$DATESTAMP.json $fio_file"
        l_and_d ""
        ...
        salt -G 'pd-node-type:DSX' cmd.run "scp /home/serviceadmin/$hssevar-$companyvar/\$HOSTNAME-dsxnfs.tgz serviceadmin@data-cluster:/home/serviceadmin/$hssevar-$companyvar/\$HOSTNAME-dsxnfs.tgz"
        l_and_d ""
        ...
        salt -G 'pd-node-type:DSX' cmd.run "scp /home/serviceadmin/$hssevar-$companyvar/\$HOSTNAME-dsxlocal.tgz serviceadmin@data-cluster:/home/serviceadmin/$hssevar-$companyvar/\$HOSTNAME-dsxlocal.tgz"
        l_and_d ""
